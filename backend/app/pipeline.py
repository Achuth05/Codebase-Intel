...
def run_pipeline(github_url: str, user_id: str, progress_callback=None) -> dict:
    def update(stage, pct):
        if progress_callback:
            progress_callback(stage, pct)

    user_repos_path = get_user_repos_path(user_id).replace("\\", "/")
    os.makedirs(user_repos_path, exist_ok=True)

    # Step 1: Clone
    update("cloning", 5)
    repo_path = clone_repo(github_url, clone_dir=user_repos_path)
    ...
    # Step 4: Build and save graph to Supabase
    update("building graph", 45)
    repo_name = get_repo_name(repo_path)
    G = build_graph(parsed_files, repo_path)
    graph_path = save_graph(G, repo_name, user_id=user_id)

    # Step 5: Chunk
    update("chunking files", 55)
    chunks = chunk_all_files(parsed_files, repo_path)

    # Step 6: Embed and store in Supabase
    update("storing embeddings", 60)
    store_chunks(chunks, repo_name, user_id=user_id)
